...
public class DicLibraryTest {
	...
	public void keyTest(){
		...
		DicLibrary.insert(key, "2015年第三季度", "我是词性", 1000);
		Result parse = DicAnalysis.parse("2015年第三季度这是用户自定义词典增加新词的例子",DicLibrary.gets(key));
		System.out.println(parse);
		boolean flag = false;
		for (Term term : parse) {
			flag = flag || "2015年第三季度".equals(term.getName());
		}
		Assert.assertTrue(flag);
	}
}